...
export const updateProduct = async (req, res) => {
  ...
  try {
    ...
    console.log("[UPDATE PRODUCT] req.files:", req.files?.length || 0, "files");
    console.log("[UPDATE PRODUCT] Full req.body:", req.body);
    ...
    console.log("[UPDATE PRODUCT] Updating with:", {
      ...productData,
      shortSpecs: "parsed",
      detailSpecs: "parsed",
    });
    ...
    if (Array.isArray(parsedUsageIds) && parsedUsageIds.length > 0) {
      ...
      if (invalidUsageIds.length > 0) {
        console.warn(
          `[UPDATE PRODUCT] Invalid usage IDs: ${invalidUsageIds.join(", ")}`
        );
      }
      ...
      // Xóa tất cả usages nếu không có usageIds
      await product.setUsages([], { transaction });
    }

    // 7. COMMIT TRANSACTION
    await transaction.commit();

    // 8. LẤY PRODUCT VỚI THÔNG TIN ĐẦY ĐỦ
    const productWithDetails = await Product.findByPk(product.id, {
      include: [
        { model: Brand, attributes: ["id", "name"] },
        { model: Series, attributes: ["id", "name"] },
